#!/usr/bin/python
#./simuHybrid_schumer_model_disperseArchi.py 0.1 0.1 0.5 0.1 50
'''
...
if len(sys.argv[1:]) == 5:
    ...
    print "\nUsage: <simuHybrid_schumer_model_disperseArchi.py> s_dmi(float) s_add(float) h(float) r(float) n(int)" 
...
my_dir_string = dname+'/simuHyb_disperseArch_n'+str(popsize)+'_'+time.strftime("%d-%m-%Y")
call(["mkdir", my_dir_string])
out = open(my_dir_string+'/simuHyb_disperseArch_'+time.strftime("%d-%m-%Y")+'_data_sDMI'+str(s_dmi)+'_sADD'+str(s_add)+'_r'+str(r)+'.txt', "w")

# write header line for dispersed genetic architecture:
out.write('gen\tdmi1\tdmi1_lnkd\tneu1\tadapt1\tadapt1_lnkd\tneu2\tdmi3\tdmi3_lnkd\tneu3\tadapt2\tadapt2_lnkd\tneu4\tdmi2\tdmi2_lnkd\tneu5\tadapt3\tadapt3_lnkd\tneu6\tdmi4\tdmi4_lnkd\n')
out.close()
    ...
    pop.evolve(
        ...
        postOps=[
            ...
            sim.PyEval(r"'%d\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n' % "
                ...
                output = '>>>'+my_dir_string+'/simuHyb_disperseArch_'+time.strftime("%d-%m-%Y")+'_data_sDMI'+str(s_dmi)+'_sADD'+str(s_add)+'_r'+str(r)+'.txt')
        ], 
        gen = 1001
    )
