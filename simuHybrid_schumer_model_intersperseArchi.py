#!/usr/bin/python
#./simuHybrid_schumer_model_intersperseArchi.py 0.1 0.1 0.5 0.1 50
'''
...
if len(sys.argv[1:]) == 5:
    ...
    print "\nUsage: <simuHybrid_schumer_model_intersperseArchi.py> s_dmi(float) s_add(float) h(float) r(float) n(int)" 
...
my_dir_string = dname+'/simuHyb_intersperseArch_n'+str(popsize)+'_'+time.strftime("%d-%m-%Y")
call(["mkdir", my_dir_string])
out = open(my_dir_string+'/simuHyb_intersperseArch_'+time.strftime("%d-%m-%Y")+'_data_sDMI'+str(s_dmi)+'_sADD'+str(s_add)+'_r'+str(r)+'.txt', "w")

# write header line for interspersed genetic architecture:
out.write('gen\tdmi1\tdmi1_lnkd\tneu1\tadapt1\tadapt1_lnkd\tneu2\tdmi2\tdmi2_lnkd\tneu3\tadapt2\tadapt2_lnkd\tneu4\tdmi3\tdmi3_lnkd\tneu5\tadapt3\tadapt3_lnkd\tneu6\tdmi4\tdmi4_lnkd\n')
out.close()
...
for rep in range(1,6):
    pop = sim.Population(size=popsize, ploidy = 2, loci=[20], lociPos=[1,1.1,2,3,3.1,4,5,5.1,6,7,7.1,8,9,9.1,10,11,11.1,12,13,13.1], infoFields=['fitness'])
    sim.initGenotype(pop, genotype=[0]*20+[1]*20)  # all individuals are heterozygous across the genome
    
    # extraordinarily clunky and bruit-force function used to determine relative fitness based on genotype.
    def sel(geno):
    ...
    pop.evolve(
        ...
        postOps=[
            ...
            sim.PyEval(r"'%d\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n' % "
                ...
                output = '>>>'+my_dir_string+'/simuHyb_intersperseArch_'+time.strftime("%d-%m-%Y")+'_data_sDMI'+str(s_dmi)+'_sADD'+str(s_add)+'_r'+str(r)+'.txt')
        ], 
        gen = 1001
    )
