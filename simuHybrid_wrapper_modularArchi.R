#!/usr/bin/env Rscript
#./simuHybrid_wrapper_modularArchi.R

#############################################################
# wrapper to run "simuHybrid_schumer_model_modularArchi.py" #
...
# call the "simuHybrid_schumer_model_modularArchi.py" script
  ...
  for (rec.rate in rs) {
    for (s_dmi in s_dmis) {
      for (s_add in s_adds) {
        ...
        system(paste("./simuHybrid_schumer_model_modularArchi.py",  s_dmi, s_add, h, rec.rate, n, sep=" "))
